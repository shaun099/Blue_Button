...
// /**
//  * Transform Outpatient Institutional EOB from FHIR to structured JSON
//  * @param {Object} rawFHIR
//  * @returns {Object}
//  */
...
//   // Map contained organizations (e.g. provider)
//   const providerOrg = (claim.contained || []).find(o => o.resourceType === 'Organization');
...
//       ndcCode: getValue(item, 'productOrService.extension.0.valueCoding.code'),
//       quantity: getValue(item, 'quantity.value'),
//       location: getValue(item, 'locationAddress.state'),
//       modifiers: (item.modifier || []).map(m => getValue(m, 'coding.0.code')),
...
//         paidToProvider: findAmount(adj, 'paidtoprovider'),
//         paidToPatient: findAmount(adj, 'paidtopatient'),
//         coinsurance: findAmount(adj, 'coinsurance'),
//         deductible: findAmount(adj, 'deductible'),
//         nonCovered: findAmount(adj, 'noncovered'),
//         paidByPatient: findAmount(adj, 'paidbypatient')
//       },

//       denialReasons: adj
//         .filter(a => a.category?.coding?.some(c => c.code === 'denialreason'))
//         .map(a => getValue(a, 'reason.coding.0.code'))
//     };
//   });

//   // Totals
//   const totals = {
//     submitted: claim.total?.find(t =>
...
//     date: claim.created,
//     method: totals.submitted > 0 ? 'Paid via Medicare' : 'Unknown'
//   };
...
//       },
//       controlNumber: getExtensionValue(claim.extension, 'https://bluebutton.cms.gov/resources/variables/fi_doc_clm_cntl_num')?.valueIdentifier?.value
//     },
//     patient: {
//       reference: getValue(claim, 'patient.reference')
//     },
//     providers: {
//       billingProvider: providerOrg?.name,
//       billingNPI: providerOrg?.identifier?.find(i => i.system?.includes('us-npi'))?.value,
//       careTeam: careTeam
//     },
//     insurance: {
